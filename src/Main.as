package 
{
	import benkuper.ui.fonts.Fonts;
	...
	public class Main extends Sprite 
	{
		...
		public function Main():void 
		{
			...
			network = new NeuralNetwork([numInputs, 8,8,1]);
			addChild(network);
		...
		private function mouseEnterFrame(e:Event):void 
		{
			...
			for (var i:int = 0; i < trainingInputValues.length; i++)
			{
				...
				if (d < .1) expectedValues[i][0] = Math.min(expectedValues[i][0]+.3,1);
		...
		{
			var numTrainingValues:int = 4000;
		...
		public function drawTrainingValues(checkRun:Boolean = true,newPoints:Boolean = false):void
		{
			...
			trainingContainer.graphics.beginFill(0x4B4B4B);
			...
			checkContainer.graphics.beginFill(0x4B4B4B);
			...
			for (var i:int = 0; i < numTest; i++)
			{
				...
				if (checkRun)
				{
					...
					if (newPoints)
					{
						...
						checkContainer.graphics.beginFill(lerpColor(0xF05513, 0x87E320, outValues[0]));
						checkContainer.graphics.drawRect(100+p.x * 100, 100+p.y * 100,1,1);
						checkContainer.graphics.endFill();
					}else
					{
						outValues = network.run(trainingInputValues[i]);
						checkContainer.graphics.lineStyle(1,lerpColor(0xF05513, 0x87E320, outValues[0]));
						checkContainer.graphics.moveTo(99 + trainingInputValues[i][0] * 100, 100 + trainingInputValues[i][1] * 100);
						checkContainer.graphics.lineTo(100 + trainingInputValues[i][0] * 100, 100 + trainingInputValues[i][1] * 100);// 1, 1);
						checkContainer.graphics.endFill();
					}
				...
				}else
				{
					...
					trainingContainer.graphics.beginFill(lerpColor(0xF05513, 0x87E320, expectedValues[i][0]));
					trainingContainer.graphics.drawRect(100+trainingInputValues[i][0] * 100, 100+trainingInputValues[i][1] * 100,1,1);
					trainingContainer.graphics.endFill();
